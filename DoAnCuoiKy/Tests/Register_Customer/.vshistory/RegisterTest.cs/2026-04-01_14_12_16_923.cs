...
namespace DoAnCuoiKy.Tests.Register_Customer
{
    public class RegisterTest
    {
        ...
        public void RegisterAccountTest(string tcId, string? objective, List<TestStep> steps, string? expectedResult, int startRow)
        {
            ...
            switch (objective)
            {
                case string s when s.Contains("email trùng"):
                    data = "Email";
                    break;
                ...
                case string s when s.Contains("số điện thoại"):
                    data = "Phone";
                    break;

                case string s when s.Contains("mật khẩu"):
                    data = "Password";
                    break;

                case string s when s.Contains("xác nhận mật khẩu"):
                    data = "ConfirmPassword";
                    break;

                case string s when s.Contains("điều khoản"):
                    data = "Terms";
                    break;

                case string s when s.Contains("họ"):
                    data = "FirstName";
                    break;

                case string s when s.Contains("tên"):
                    data = "LastName";
                    break;
            }
        ...
        private void ExecuteStep(TestStep step, TestStep? nextStep)
        {
            ...
            else if (action.Contains("không chấp nhận điều khoản"))
                registerPage.DoNotThing();
            else if (action.Contains("chấp nhận điều khoản"))
        ...
        private string GetActualResult(string data)
        {
            ...
            string validation = registerPage.GetValidationErrors(data);

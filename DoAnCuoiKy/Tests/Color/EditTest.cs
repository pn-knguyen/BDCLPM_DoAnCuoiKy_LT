...
namespace DoAnCuoiKy.Tests.Color
{
    public class EditTest
    {
        ...
        private EditColor colorPage;
        private const string SheetName = "Test Cases AD";
        private const string TestCaseFilter = "F5.2_";
        ...
        public void Setup()
        {
            ...
            colorPage = new EditColor(driver, wait);
        }
        ...
        [Test, TestCaseSource(typeof(ExcelDataProvider), nameof(ExcelDataProvider.GetTestCases), new object[] { "Test Cases AD", "F5.2_" })]
        public void EditColorTest(string tcId, string? objective, List<TestStep> steps, string? expectedResult, int startRow)
        {
        ...
        private void ExecuteStep(TestStep step, TestStep? nextStep)
        {
            ...
            else if (action.Contains("màu cần sửa"))
                colorPage.ClickEditColor(data);
            else if (action.Contains("nhập mã màu"))
                colorPage.EnterColor(data);
            else if (action.Contains("sửa màu sắc") || action.Contains("lưu"))
                colorPage.Submit();
        }
        ...
        private string GetActualResult(string data)
        {
            ...
            // If the updated color value appears in the list after editing, treat as successful update
